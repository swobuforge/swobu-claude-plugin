...
: "${ANTHROPIC_API_KEY:?E2E requires ANTHROPIC_API_KEY}"
...
case "$MODE" in
  ...
  connect-unknown | connect-invalid | connect-one | connect-idempotent | \
  connect-replace | request-smoke)
    run_case "$MODE" "$MODE"
    ;;
  all)
    ...
    run_case e7-invalid connect-invalid
    run_case e8-idempotent connect-idempotent
    run_case e9-replacement connect-replace
