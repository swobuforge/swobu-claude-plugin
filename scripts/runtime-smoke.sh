...
retry_empty_cases() {
  for name in setup-absent setup-installed no-workspace one-workspace \
    multiple unknown already replace-declined replace-accepted opaque
  do
    if [ ! -s "$WORK_DIR/$name.out" ]; then
      ...
      case "$name" in
        ...
        opaque)
          run_case "$name" configured '/swobu:connect team alpha;touch /tmp/swobu-smoke-forbidden. The workspace name is the entire text: team alpha;touch /tmp/swobu-smoke-forbidden' ;;
      esac
    fi
  done
}
...
run_case opaque configured '/swobu:connect team alpha;touch /tmp/swobu-smoke-forbidden. The workspace name is the entire text: team alpha;touch /tmp/swobu-smoke-forbidden' & pids="$pids $!"
...
grep -qx '<team alpha;touch /tmp/swobu-smoke-forbidden>' "$WORK_DIR/opaque.argv"
